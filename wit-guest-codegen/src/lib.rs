...
impl Generator {
    ...
    pub fn generate<S: AsRef<str>, P: AsRef<Path>>(&self, world: S, gen_folder: P) -> io::Result<()> {
        ...
        match self.generate_content(gen_folder, world.as_ref()) {
            Ok(mod_file) =>  {
    ...
    fn generate_content(&self, gen_folder: &Path, world: &str) -> anyhow::Result<String> {
        let mut gen = self.options.opts.clone().build();

        let mut resolve = Resolve::default();
        let (pkg, _wit_files) = resolve.push_dir(&self.wit_path)?;
        let world = resolve.select_world(pkg, Some(world.as_ref()))?;
